...
from src.data.config import AMP_EMB_HIDDEN
from src.data.config import CHECKPOINT_DIR
from src.data.config import DEVICE
from src.data.config import GRADIENT_CLIP
from src.data.config import HPO_CFG
from src.data.config import PHASE_EMB_HIDDEN
from src.data.config import RANDOM_SEED
from src.data.config import SCHEDULER_CFG
from src.data.config import TRAINING
from src.data.config import VAL_SPLIT
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            for Xb, Ab in loaders['val']:
                t_norm, theta = Xb[:, :1].to(device), Xb[:, 1:].to(device)
                Ab = Ab.to(device)
                val_loss += criterion(amp_model(t_norm,
                                      theta), Ab) * Xb.size(0)
        val_loss /= len(val_ds)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            for Xb, Phib in loaders['val']:
                t_norm, theta = Xb[:, :1].to(device), Xb[:, 1:].to(device)
                val_loss += criterion(phase_model(t_norm, theta),
                                      Phib.to(device)) * Xb.size(0)
        val_loss /= len(val_ds)
        scheduler.step(val_loss)
...
def objective_amp(trial):
    ...
    amp_size = trial.suggest_categorical(
        'amp_hidden_size', [64, 128, 256, 512])
...
def objective_phase(trial):
    lr = trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True)
    phase_size = trial.suggest_categorical(
        'phase_hidden_size', [64, 128, 256, 512])
    banks = trial.suggest_int('banks', 1, 6)
    dropout = trial.suggest_float('dropout', 0.0, 0.5, step=0.05)
    num_layers = trial.suggest_int('layers', 3, 6)
    weight_decay = trial.suggest_float('weight_decay', 1e-8, 1e-2, log=True)
