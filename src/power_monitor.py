...
def read_amd_power() -> Optional[float]:
    ...
    """
    try:
        output = subprocess.check_output(["rocm-smi", "--showpower"], stderr=subprocess.DEVNULL)
            ...
            if "Average Graphics Package Power" in line:
                ...
                for p in parts:
                    if p.endswith("W"):
                        ...
                        logging.debug(f"[PowerMonitor] AMD power reading: {power} W")
                        return power
...
class PowerMonitor:
    ...
    def _sample_loop(self):
        ...
        while not self._stop_event.is_set():
            if NVML_AVAILABLE and _handle is not None:
                ...
                self.logger.debug(f"AMD GPU power usage (or fallback): {power_w:.3f} W")
    ...
    def __enter__(self):
        if NVML_AVAILABLE:
            self.logger.info(f"NVIDIA GPU detected. Monitoring every {self.interval}s.")
        else:
    ...
    def summary(self) -> dict:
        ...
        self.logger.info(f"PowerMonitor summary: mean={mean_power:.2f} W, max={max_power:.2f} W, min={min_power:.2f} W over {num_samples} samples")
        return {
            "mean_w": mean_power,
            "max_w": max_power,
            "min_w": min_power,
